...
# @Date:   2019-09-25 13:46:14
# @Breif: 
# @Last Modified by:   Alfred Xiang Wu
# @Last Modified time: 2019-09-26 09:15:32
import os
import time
import argparse
import numpy as np
...
parser.add_argument('--test_epoch', default=120, type=int)
parser.add_argument('--save_epoch', default=1, type=int)

parser.add_argument('--ip_model', default='', type=str)
parser.add_argument('--img_root', default='', type=str)
parser.add_argument('--train_list', default='', type=str)

parser.add_argument('--loss_mmd',  default=10, type=float)
parser.add_argument('--loss_ip',  default=10, type=float)
parser.add_argument('--loss_pair',  default=0.5, type=float)
...
def main():
    ...
    for epoch in range(start_epoch, args.all_epochs):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            img_nir = batch['0'].cuda()
            img_vis = batch['1'].cuda()
            ...
            # mmd loss
            loss_mmd = args.loss_mmd * torch.abs(z_nir.mean(dim=0) - z_vis.mean(dim=0)).mean()
            ...
            loss_ip = args.loss_ip * (criterionL2(rec_nir_fc, nir_fc.detach()) + criterionL2(rec_vis_fc, vis_fc.detach())) / 2.0
            loss_pair = args.loss_pair * criterionL2(rec_nir_fc, rec_vis_fc)
        ...
        if (epoch % args.test_epoch is 0 or epoch == 1):
            ...
            vutils.save_image(nir.data, '{}/Epoch_{:03d}_id.png'.format(args.out_path, epoch))
            vutils.save_image(vis.data, '{}/Epoch_{:03d}_photo.png'.format(args.out_path, epoch))
            vutils.save_image(rec_nir.data, '{}/Epoch_{:03d}_rec_id.png'.format(args.out_path, epoch))
            vutils.save_image(rec_nir.data, '{}/Epoch_{:03d}_rec_photo.png'.format(args.out_path, epoch))
            vutils.save_image(fake[:, 0:3, :, :].data, '{}/Epoch_{:03d}_fake_id.png'.format(args.out_path, epoch))
            vutils.save_image(fake[:, 3:6, :, :].data, '{}/Epoch_{:03d}_fake_photo.png'.format(args.out_path, epoch))
